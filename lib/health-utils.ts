...
export const getNewHealthDeposit =  (depositAmountBN : BigNumber, UserAccountData: IAccountData, assetPrice: BigNumberish, tokenSymbol: string ) => {
  if(UserAccountData?.currentLiquidationThreshold && UserAccountData?.totalCollateralETH && UserAccountData?.totalDebtETH && depositAmountBN && assetPrice && tokenSymbol){    
    ...
    if(!UserAccountData.totalDebtETH.isZero()){
      newHealth = (newCollateralETH.mul(UserAccountData?.currentLiquidationThreshold)).div(100)?.div(UserAccountData.totalDebtETH);
    }else{
...
//down
export const getNewHealthWithdraws =  (withdrawAmountBN : BigNumber, UserAccountData: IAccountData, assetPrice: BigNumberish, tokenSymbol: string ) => {
  if(UserAccountData?.currentLiquidationThreshold && UserAccountData?.totalCollateralETH && UserAccountData?.totalDebtETH && withdrawAmountBN && assetPrice && tokenSymbol){
...
export const getMaxWithdraws =  (UserAccountData: IAccountData, assetPrice: BigNumberish, tokenSymbol: string, decimals: string) => {
  if(UserAccountData?.currentLiquidationThreshold && UserAccountData?.totalCollateralETH && UserAccountData?.totalDebtETH && assetPrice && tokenSymbol){
    ...
    let debtDivLiquid;
    if(!UserAccountData?.currentLiquidationThreshold.isZero()){
      debtDivLiquid = healthMulDebt.mul(10000).div(UserAccountData?.currentLiquidationThreshold);
    }else{
...
//down
export const getNewHealthBorrows =  (borrowAmountBN : BigNumber, UserAccountData: IAccountData, assetPrice: BigNumberish, tokenSymbol: string ) => {
  if(UserAccountData?.currentLiquidationThreshold && UserAccountData?.totalCollateralETH && UserAccountData?.totalDebtETH && borrowAmountBN && assetPrice && tokenSymbol){
    ...
    if(!newDebtETH.isZero()){
      newHealth = (UserAccountData?.totalCollateralETH.mul(UserAccountData?.currentLiquidationThreshold)).div(100)?.div(newDebtETH);
    }else{
...
export const getMaxBorrows =  (UserAccountData: IAccountData, assetPrice: BigNumberish, tokenSymbol: string, decimals: string ) => {
  if(UserAccountData?.currentLiquidationThreshold && UserAccountData?.totalCollateralETH && UserAccountData?.totalDebtETH && assetPrice && tokenSymbol){
    const assetEthPrecision = utils.parseUnits(assetPrice.toString(), '18' );
    const maxHealthFactorPrecision = utils.parseUnits('1.1', '18'); 

    const collatMulLiquid = UserAccountData.totalCollateralETH.mul(UserAccountData?.currentLiquidationThreshold).div(10000);
...
//up
export const getNewHealthRepay =  (repayAmountBN : BigNumber, UserAccountData: IAccountData, assetPrice: BigNumberish, tokenSymbol: string ) => {
  if(UserAccountData?.currentLiquidationThreshold && UserAccountData?.totalCollateralETH && UserAccountData?.totalDebtETH && repayAmountBN && assetPrice && tokenSymbol){
